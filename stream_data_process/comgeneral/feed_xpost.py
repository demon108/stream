...
SOURCE_TYPE_MAP = {'news':1,'N':1,'forum':2,'blog':3,'qa':6,'weixin':12,'SNS':8} 
...
def feed_data_to_xpost(conn,qualified_datas,terrace):
	...
	try:
        ...
        try:
            ...
            open('xentryid_error.log','a+').write('%s\t%s\n'%(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),str(e)))
            return 0
        xpostnum = xentryid%8
        title,abstract,posttime,url,author,comment_count,click_count,template_type = qualified_data['title'],'',qualified_data['pubtime'],qualified_data['url'],qualified_data.get('author',''),0,0,qualified_data['pagetype']
	    ...
	    #print xpostnum,postid
            open('duplicate_sql','a+').write('%s\t%s\n'%(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),duplicate_sql))
            continue
